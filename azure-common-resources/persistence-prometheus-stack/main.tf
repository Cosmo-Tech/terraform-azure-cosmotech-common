...
resource "azurerm_managed_disk" "prometheus_master" {
  count                = var.pv_prometheus_provider == "azure" && var.pv_prometheus_disk_source_existing ? 0 : 1
...
resource "kubernetes_persistent_volume" "pv_prometheus_master" {
  ...
  depends_on = [
    azurerm_managed_disk.prometheus_master,
    data.azurerm_managed_disk.disk_managed_prometheus
  ]
}
...
data "azurerm_managed_disk" "disk_managed_grafana" {
  count               = var.pv_grafana_provider == "azure" && var.pv_grafana_disk_source_existing ? 1 : 0
  name                = local.disk_grafana_name
  resource_group_name = var.kubernetes_mc_resource_group_name
}
